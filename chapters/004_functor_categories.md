...
have `alpha, beta :: Arr [C, D]`, `alpha : F -> G`, `beta : G -> H`, we can obtain `alpha . beta : F -> H`. Let us call
...
an operation that transforms all `F x` to `G x` for `x :: C`, and all `F f` to `G f` for `f :: Arr C`. We will denote
the resulting natural transformation `alpha`, its transformation of `x` as `alpha_x` and its transformation of `f`
as `alpha_f`.

Given any `f :: Arr C`, we need to establish what `alpha_f : F f -> G f`, does. Note that given `x :: C`,
`F x, G x :: D` so `alpha_x :: Arr D`. Also note that if `f :: Arr C`, `F f, G f :: Arr D`. In other words,
`F f, G f, alpha_x, alpha_y :: Arr D`.
...
Then, for any `f :: Arr C`, `f : x -> y`, we have `F x :: D`. We can now trace the following path:
...
Since we have a unique `alpha_f : F f -> G f` for any `f :: Arr C`, `f : x -> y`, this can be envisioned as a
unique `f' :: Arr (D * D)` from `(F x, F y)` to `(G x, G y)`. This implies the value of `G y` is uniquely determined
